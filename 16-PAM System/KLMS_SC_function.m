...
expansionCoefficient = stepSize*conj(trainTarget(1)); 
...
for n = 2:trainSize
    ...
    netSizeDiagram(n) = netSizeDiagram(n-1) + 1;
        ...
        expansionCoefficient(dictSize) = stepSize*conj(trainTarget(n)- networkOutput);
    
        y_out = zeros(trainSize,1);
        for jj = 1:trainSize   
            y_out(jj) = conj(expansionCoefficient)*...
