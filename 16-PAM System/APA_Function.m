function [coeffesAPA,learningCurve] = APA_Function(train_input_ML,train_target,dimen,K,stepSize,paramRegularization)
        ...
        for ii=1:N
            Output = train_input_ML(:,ii).'*coeffesAPA;
            if ii<K
                coeffesAPA= coeffesAPA+stepSize*(train_target(ii)-Output)*train_input_ML(:,ii);
            else
             X=train_input_ML(:,ii-K+1:ii);
             Y=train_target(ii-K+1:ii);
            ei=Y.'-X.'*coeffesAPA;
            coeffesAPA= coeffesAPA+stepSize*X*pinv(X.'*X+paramRegularization*eye(K))*ei;
             end
        y_out = zeros(N,1);
        for jj = 1:N
            y_out(jj) = train_input_ML(:,jj).'*coeffesAPA;
        end
